...
/*   Updated: 2022/12/26 12:48:04 by brda-sil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_is_line_is_map(char *l, unsigned char *player)
{
	...
	while (l[i])
	{
		if ((l[i] == 'N' || l[i] == 'S' || l[i] == 'W' || l[i] == 'S') \
			&& *player == 0)
			*player = 1;
		else if ((l[i] == 'N' || l[i] == 'S' || l[i] == 'W' || l[i] == 'S') \
			&& *player == 1)
			return (1);
		else if (l[i] != '1' && l[i] != '0' && l[i] != ' ' && l[i] != '\n')
			return (2);
		i++;
	}
	return (-1);
...
int	parse_map(char *line, t_parse *main)
{
	...
	while (ft_isblank(line[i]))
		i++;
	if (in_map == 1 && fill_map(line, main) == 1)
		return (15);
