...
/*   Updated: 2023/01/28 06:24:35 by brda-sil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cast_ray_horizontal(t_ray *ray, t_player player, t_map map)
{
	...
	if (ray->angle == 0 || ray->angle == PI)
	{
		...
		ray->depth_of_field = map.size.y + FOG + 1;
	}
	while (ray->depth_of_field < map.size.y + FOG + 1)
	{
		ray->max.x = (unsigned long int)(ray->pos.x) >> ray->bit_prec;
		ray->max.y = (unsigned long int)(ray->pos.y) >> ray->bit_prec;
		if (!ray_hit(ray, map, map.size.y + FOG + 1))
			increase_offset(ray);
	}
